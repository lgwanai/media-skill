...
### 5. 🎙️ 专属声音克隆与智能配音
在本地建立你自己的“专属音色库”。只需几秒钟的样本，即可克隆你的声音。在后续配音时，支持**智能拆分超长文本**并**多线程极速并发生成**。AI 还会根据你输入的文本情境，自动调整语速和情绪，甚至**自动润色加入“哈、呢”等自然语气词**，让配音听起来极度真实。同时支持**异步状态轮询**，防超时卡死。
...
# ---------------------------------------------------------
# 5. IndexTTS-2 配置 (用于 TTS 配音与声音克隆)
# ---------------------------------------------------------
# 运行模式：'api' (使用 SiliconFlow 云端接口) 或 'local' (本地推理)
INDEXTTS_MODE = local
# 如果使用 api 模式，请填写 SiliconFlow API Key
...
> ⚠️ **提示**：配音可能比较耗时，如果您是通过 Agent（如 Claude Code/OpenClaw）调用，建议使用 `nohup` 或 `&` 放到后台执行，防止超时卡死。

**🔥 新特性：自然语气/副语言标签支持！**
为了让 AI 说话更像真人，你可以直接在配音文本中插入以下副语言标签，AI 会自动将其转化为极度逼真的笑声或停顿，而**不会把标签当成文字读出来**。
*目前支持的标签包括：*
- `[laughter]` 或 `[laugh]`：自然笑声（结合“高兴”情绪向量效果极佳）
- `[breath]`：自然呼吸声/换气停顿
- `[sigh]`：叹气声
- `[cough]`：咳嗽声
- `[cry]`：哭腔（需配合“悲伤”情绪向量）
- `[smack]`：嘴唇吧唧声
- `[pause]` 或 `[uv_break]`：强制自然停顿

*示例用法：*
```bash
# 直接让 AI 念出这段话，包含笑声和停顿
nohup python scripts/dubbing.py dub --text "大家晚上好啊[laughter]，今天我们要讲一个很有趣的话题[pause]，那就是..." --out output.mp3 > dubbing.log 2>&1 &

# 或者，直接根据做好的字幕文件配音
nohup python scripts/dubbing.py dub --srt <你的字幕文件.srt> --out output.mp3 > dubbing.log 2>&1 &
```
