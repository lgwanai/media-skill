...
**⚠️ 重要提示：长音频/视频识别非常耗时，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/transcribe.py <音视频文件路径> --async-run
...
**⚠️ 重要提示：涉及视频切片处理非常耗时，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/extract_by_theme.py <音视频文件路径> "<主题描述>" --async-run
...
**⚠️ 重要提示：视频处理可能耗时，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/clip_video.py output/<视频名>/transcription.json <待剪辑视频路径.mp4> --async-run
...
**⚠️ 重要提示：涉及视频上传和多模态大模型分析，耗时较长，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/analyze_video.py <待分析的视频路径.mp4> --async-run
...
```bash
# 克隆音色 (添加 --async-run 参数可直接后台异步执行，防止 Agent 卡死)
python scripts/dubbing.py clone --audio <参考音频.mp3> --name <自定义音色名称> --async-run
...
**⚠️ 重要提示：长文本配音非常耗时，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回，避免 Agent 卡死超时！**

```bash
# 方式一：根据字幕文件配音（添加 --async-run 自动后台执行）
python scripts/dubbing.py dub --srt <字幕文件.srt> --voice <音色名> --out <输出音频.mp3> --async-run
# 查询状态: cat output/<输出音频所在目录>/dubbing_status.json

# 方式二：根据本地文本文件进行配音（推荐！彻底避免命令行长文本转义失败问题，添加 --async-run 自动后台执行）
python scripts/dubbing.py dub --text-file "长文本.md" --voice <音色名> --out <输出音频.mp3> --async-run
# 查询状态: cat output/<输出音频所在目录>/dubbing_status.json

# 方式三：直接输入纯文本配音（支持超长文本与副语言标签，添加 --async-run 自动后台执行）
python scripts/dubbing.py dub --text "大家晚上好啊[laughter]，今天我们要讲一个很有趣的话题[pause]..." --voice <音色名> --out <输出音频.mp3> --async-run
# 查询状态: cat output/<输出音频所在目录>/dubbing_status.json
```
**功能特性**：
...
- **异步状态查询**：由于本地大模型加载和配音生成非常耗时，**强烈建议追加 `--async-run` 参数**，脚本会自动转入后台运行。Agent 可以**立即返回并**每隔 20 秒定期读取 `output/text_dubbing_unknown/dubbing_status.json`（或克隆音色的 `clone_status.json`）以掌握进度，彻底防止工具调用超时卡死。

### 7. 独立功能：智能配字幕与翻译
一键为视频生成字幕并压制，支持基于语境的大模型翻译、语气词过滤与自动折行排版。
**⚠️ 重要提示：视频转码和压制字幕可能比较耗时，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/subtitle.py <音视频文件路径> [--lang "目标语言，如：English"] [--out <输出视频路径>] --async-run
...
**⚠️ 重要提示：涉及视频切片和合并，耗时较长，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/highlight.py <音视频文件路径> [--outdir <输出目录>] --async-run
...
### 9. 联合处理：提取精彩片段 -> 智能剪辑 -> 配字幕
将上述能力串联，自动完成高光提取、精剪去废话、翻译并压制字幕的全流水线。
**⚠️ 重要提示：极度耗时的终极流程，Agent 调用时请务必追加 `--async-run` 参数，脚本会自动转入后台执行并立即返回！**
```bash
python scripts/combine.py <音视频文件路径> [--lang "目标语言，如：English"] [--outdir <输出目录>] --async-run
